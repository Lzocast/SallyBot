#!/usr/bin/env python3
# All credit to Bill Weinman 2017 for creation of this template
...
class numwords():
    ...
    _oor = 'OOR' # Out of Range distinction

    def __init__(self, n):
        self._number = n;

    def numwords(self, num = None):
        'Return the words as numbers'
...
class saytime(numwords):
    '''
        return the time (from two params) as words,
        eg: quarter to three etc
    '''
    _specials = {
        'noon': 'noon',
        'midnight': 'midnight',
    ...
    def words (self):
        ...
        if m == 0:
            if h in (0, 12): return hword # for noon and modnight
    ...
    def digits(self):
        'return the traditional time eg: 13:42'
        return f'{self._hour:02}:{self._min:02}'

class saytime_t(saytime): # wrapper for saytime to use object
    '''
...
def main():
    if len(sys.argv) > 1:
        ...
        print('The current time is ' + saytime_t().words())
...
def test():
    ...
    list = (
        ...
        50, 51, 52, 55, 59, 100, 101, 112, 900, 999, 1000
    )
    for l in list:
        st.number(l)
        print(l, st.numwords())
