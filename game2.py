...
def guard():
    ...
    actions_dict = {"check":"""\nDM_Sally: You see the guard is still sleeping; a faint wisp of fresh air caresses your face, coming from the door 
on the right of him. What are you waiting for?\n""",
                    "sneak":"\nDM_Sally: You approach the guard, he's still sleeping. Reaching for the door, you open it slowly... The guard shifts... and slip out.\n",
                    "attack":"""\nDM_Sally: Steeling yourself you move swiftly towards the sleeping guard, still trying to decide if you mean to land a killing blow or 
just knock him out with the hilt of your sword. Unfortunately things don't go as planned.\n""",
    ...
    while True:
        ...
        if action in actions_dict.keys():
            ...
            if action == "sneak":
                print("DM_Sally: You just slipped through the door before the guard realised it.")
                time.sleep(2)
                print("DM_Sally: You are now outside, home free! Huzzah!\n")
                return 
            elif action == "attack":
                you_died("""As you move forward to strike, your foot presses down on a loose board with a loud 'creeeeEEEEaak'. 
Woken by the noise the Guard reaches for his dagger, and as you scramble for your sword pain blossoms in your chest 
and everything goes grey... and dark... You just died.\n<GAME OVER>""")
...
def blue_door_room(t):
    ...
    '''
    #NOTE So our treasure_chest list contains 4 items RETURN TREASURE_CHEST HERE IF CODE FUCKED
    ...
    if action.lower() in ["treasure", "chest", "left"]:
        ...
        if choice == "1":
            ...
            time.sleep(3)
            print("DM_Sally: The chest creaks open... the guard stirs... snorts... and resumes his rest. That's one heavy sleeper!")
            time.sleep(4)
            ...
            if treasure_choice == "1":
                ...
                print("\tDM_Sally: You take the gleaming sword from the treasure chest. It does looks exceedingly shiney.")
                time.sleep(3)
                print("\tDM_Sally: Woohoo! Bounty and a shiney new sword. You discard your current, much crapier, sword in the chest.")
                ...
                print(f"\nDM_Sally: You close the lid of the chest containing the {treasure_chest} for the next adventurer. /grins\n")
                time.sleep(5)
                print("DM_Sally: Now onward to get past this sleeping guard and the door to freedom... ")
            elif treasure_choice == "2":
                print("DM_Sally: Treasure is all well and good, but getting out of here would be better")
        elif choice == "2":
            print("DM_Sally: Whatever is in there can wait, you need to escape")
    elif action.lower() in ["guard", "right"]:
        print("DM_Sally: Good thinking, you should probably make sure you know what is up with that Guard.")
    else:
        print("DM_Sally: Well, not sure what you picked there, soooooooo let's poke the guard cos it's fun!")
    guard()
...
def red_door_room(t):
    ...
    print(t)
    time.sleep(9)
    print("""DM_Sally: It stares at you through one narrowed eye for a moment.... and you could swear it seems to grin as a ravenous 
understanding fills that gaze.""")
    time.sleep(6)
    ...
    if "flee" in next_move:
        start_adventure("""\nDM_Sally: Reaching slowly behind you, you fumble for the door handle. Wincing as the hot metal sears your palm, you back through the opening 
and slam the door closed as soon as you are through. Heart pounding you reconsider your choices in life and decide to pretend that never happened.""")
   
        # You call the function you_died and pass the reason why you died as
        # a string as an argument.
    you_died("You're taking on a dragon with only a sword... Well, it eats you.... But at least you gave it indigestion!")
### END ROOMS ###
...
def start_adventure(t):
    ...
    door_picked = input("DM_Sally: As you ease yourself up on aching muscles, brushing away bits of damp hay from your clothes, do you pick the red door or blue door?\nYour Decision [red | blue] > ")

    # Pick a door and we go to a room and something else happens
    if door_picked == "red":
        red_door_room("""\nDM_Sally: The door grinds open on stiffened hinges... and there you see a great red dragon. Towering and magnificent, flames tickle the edge of 
nostrils the size of your head as it breathes. Teeth as long as your arm poke out from its maw... and as if sensing the stilling of your heart the beast 
swivels its scaly head to regard you.""")
    elif door_picked == "blue":
        blue_door_room("""\nDM_Sally: Opening the door slowly you see a room with a wooden treasure chest on the left, and a sleeping guard sat on a simple stool on the right 
in front of the a plain iron door""")
    else:
        print("DM_Sally: Sorry, it's either 'red' or 'blue' as the answer. Look, let's start over")
        return start_adventure("You've come to in bare stone room, on a pile of stinking hay. To your left is a red door, to your right a blue door")
...
def main():
    ...
    start_adventure("""\nDM_Sally: Your mind is fuzzy and your breathing raspy as you peel open dry eyes. In the sooty torch light you can make out four barren stone walls 
and facing each other on opposite sides of the room, two doors.  To your left is a red door, the air on that side of the room feels warm and sharp... 
To your right you see a blue door, from under which skirts a cool, if musty breeze.\n""")
